...
async def reply_stream(client, message):
    ...
    try:
        msg = await reply_message.forward(chat_id=LOG_CHANNEL)
...
async def pm_text(bot, message : Message):
    ...
    link = link if link else 'https://telegra.ph/PM-MovieZ-12-06-2'
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                files_link += "\n\n<b>𝐶𝑙𝑖𝑐𝑘 𝑜𝑛 𝑵𝑬𝑿𝑻 𝑏𝑢𝑡𝑡𝑜𝑛 𝑡𝑜 𝑠𝑒𝑒 𝑚𝑜𝑟𝑒 𝑟𝑒𝑠𝑢𝑙𝑡𝑠</b>"
            else:
                btn.append(
                    [
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
                    ...
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                [
                                [
                                InlineKeyboardButton("Start Streeming bot", url="http://t.me/filestreamerv22_bot?start=true")
                            ],[
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton(' nothing ', url='t.me/')
                  ]]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            find = search.split(" ")
            search = ""
            removes = ["in","upload", "series", "full", "horror", "thriller", "mystery", "print", "file", "send", "chahiye", "chiye", "movi", "movie", "bhejo", "dijiye", "jaldi", "hd", "bollywood", "hollywood", "south", "karo", "season", "year", "the", "a", "an", "and", "of", "on", "with", "for", "at", "into"]
            for x in find:
                if x in removes:
                    continue
                else:
            ...
            search = search.replace("shaitan","shaitaan")
            search = search.replace("saitan","shaitaan")
            ...
            if not files:
                ...
                if settings["spell_check"]:
                    ...
                    if NO_RESULTS_MSG:
                        await client.send_message(chat_id=LOG_CHANNEL, text=(script.NORSLTS.format(message.chat.title, message.chat.id, message.from_user.mention, search)))
                    return
        else:
            return
    else:
    ...
    if ENABLE_SHORTLINK and settings["button"]:
        ...
        btn = []
        for file in files:
            files_link += f"""\n\n‼️ <a href=https://t.me/{temp.U_NAME}?start=files_{file.file_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('Linkz') and not x.startswith('{') and not x.startswith('Links') and not x.startswith('@') and not x.startswith('www'), file.file_name.split()))}</a>"""
    else:
        btn = [
            [
                InlineKeyboardButton(
    ...
    if imdb:
        ...
        user = message.from_user.first_name
        cap = f"<b> {user} Here's your results ✅</b>"
    CAP[search] = cap
